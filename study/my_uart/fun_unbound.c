...
void fun_delate_mac_list(u8 (*data)[6], u8 len);

u8 unbound_list[10][6];
static u8 unbound_cnt = 0;
...
void fun_unbound_init(void)
{
    ...
    bsl_adv_delate_callback(fun_delate_mac_list);
}
void OLED_Show_unMac_List(u8 (*data)[6])
{
    char mac_list[10][12] = {0};
    for (u8 i = 0; i < unbound_cnt; i++)
    {
        for (u8 k = 0; k < 6; k++)
        {
...
int fun_unbound_process(void)
{
    ...
    if (unbound_cnt > 0) //有设备，显示地址
    {
        OLED_Show_unMac_List(unbound_list);
    }
    else
    {
        num_cnt++;
        if (num_cnt > 10)
        {
            OLED_ShowString(5, 0, "no bound device", 16);
            num_cnt = 0;
        }
    }
    return 0;
...
void fun_delate_mac_list(u8 (*data)[6], u8 len)
{
    for (u8 i = 0; i < 10; i++)
    {
        if ((memcmp(&unbound_list[i][0], data, 6) == 0))
            return;
        if (unbound_list[i][0] == 0)
        {
            memcpy(&unbound_list[i][0], data, len);
            unbound_cnt++;
            return;
        }
    }
...
int fun_led_unlight_one(void)
{
    if(unbound_cnt>0)
    bsl_adv_one_bound(&unbound_list[0],0,0);
    return -1;
}
...
static event_type_t fun_unboundevent_handle(event_type_t event)
{
    if (fun_control_sm_now == UNBOUND)
    {
        ...
        case EVENT_KEY1_SHORT_PRESSED: //选灯
            select_now++;
            if (select_now >= unbound_cnt)
                select_now = 0;
            bsl_adv_one_bound(&unbound_list[select_now], 0, 0);
            break;
        case EVENT_KEY1_LONG_PRESSED: //确认选择
            bsl_adv_one_bound(&unbound_list[select_now], 0, 1);
            break;
